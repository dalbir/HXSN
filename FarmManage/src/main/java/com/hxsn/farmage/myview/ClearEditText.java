		...
		if (touchable) {
			...
			try {
				if(MyApplication.app.servers.size()>0)
				{
					...
					{
						if(MyApplication.app.servers.get(i).isOnduty()&&MyApplication.app.servers.get(i).getPhone().length()>0)
						{
							Intent inte=new Intent(Intent.ACTION_CALL,Uri.parse("tel:"+MyApplication.app.servers.get(i).getPhone()));
